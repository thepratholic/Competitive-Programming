...
def solve():
    ...
    candidate_limits = set(sugar)
    candidate_limits.add(0)

    max_satisfaction = 0

    for l_val in candidate_limits:
        current_tastiness_sum = 0
        for i in range(n):
            if sugar[i] <= l_val and tastiness[i] > 0:
                current_tastiness_sum += tastiness[i]
        
        current_satisfaction = current_tastiness_sum - l_val
