...
def solve():
    ...
    # band baj gayi karte karte, tough
    sum_total = n % MOD * pow(m, n - 1, MOD) % MOD * (m % MOD * ((m + 1) % MOD) % MOD * inv2 % MOD) % MOD

    min_total = 0
    for k in range(1, m + 1):
        cnt = (pow(m - k + 1, n, MOD) - pow(m - k, n, MOD)) % MOD
        min_total = (min_total + k * cnt) % MOD

    print((sum_total - min_total) % MOD)
