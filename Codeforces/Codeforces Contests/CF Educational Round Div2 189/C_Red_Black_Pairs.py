...
def solve():
    ...
    for j in range(1, n + 1):
        vert_cost = 1 if row1[j-1] != row2[j-1] else 0
        dp[j] = min(dp[j], dp[j-1] + vert_cost)
        
        if j >= 2:
            horiz_cost = (1 if row1[j-2] != row1[j-1] else 0) + (1 if row2[j-2] != row2[j-1] else 0)
            dp[j] = min(dp[j], dp[j-2] + horiz_cost)
