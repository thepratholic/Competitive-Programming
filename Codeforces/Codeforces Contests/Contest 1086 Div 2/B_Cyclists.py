...
def solve():
    ...
    # yar confusing hai bahot

    win_pos = p - 1        
    win_cost = a[win_pos]               
    
    before = sorted(a[:win_pos])          
    others = sorted(a[:win_pos] + a[win_pos+1:])
    
    first = max(0, p - k)   
    repeat = max(0, n - k)   
    
    first_cost = sum(before[:first]) + win_cost  
    repeat_cost = sum(others[:repeat]) + win_cost   
    
    if m < first_cost:
        print(0)
        return
    
    print(1 + (m - first_cost) // repeat_cost)
