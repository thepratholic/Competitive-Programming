...
def answer():
    ...
    c , ans = len(digits) , float('inf')
    
    #Case 1
    if(c > 1):
        ans = min(ans , a - (int(str(d[-1]) * (c - 1))))

    #Case 2
    if(d[0] > 0):
        ans = min(ans , (int(str(d[0]) * (c + 1)) - a))
    else:
        if(n == 1):ans = min(ans , a)
        else:
            num = int(str(d[1]) + ('0' * c))
            ans = min(ans , num - a)
    ...
    for i in range(c):
        ...
        if(curd != -1):
            curnum = ("".join(num)) + str(curd) + (str(d[-1]) * (c - (i + 1)))
            curnum = int(curnum)
            ans = min(ans , a - curnum)
        ...
        if(curd != -1):
            curnum = ("".join(num)) + str(curd) + (str(d[0]) * (c - (i + 1)))
            curnum = int(curnum)
            ans = min(ans , curnum - a)
