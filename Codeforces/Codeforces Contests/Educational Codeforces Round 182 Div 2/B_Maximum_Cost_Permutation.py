...
def solve():
    ...
    z = [i+1 for i,x in enumerate(a) if x==0]   
    p = {x for x in a if x>0}                   
    m = set(range(1,n+1))-p                     
    f = [i+1 for i,x in enumerate(a) if x>0 and x!=i+1]

    if not z: 
        if not f: print(0)
        else: print(max(f)-min(f)+1)
        ...
        if val==pos: 
            ...
            L = pos if not f else min(pos,min(f))
            R = pos if not f else max(pos,max(f))
            print(R-L+1)
