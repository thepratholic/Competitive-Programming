...
class DSU:
    ...
    def union(self, u, v):
        parent_u = self.findUPar(u)
        parent_v = self.findUPar(v)
...
def solve():
    ...
    for i in range(n):
        if dsu.findUPar(i) == i:
            ultimate_parents.append(i)

    print(len(ultimate_parents) - 1)

    for cur in range(len(ultimate_parents) - 1):
        i, j = redundant[cur]

        u = ultimate_parents[cur]
        v = ultimate_parents[cur + 1]
