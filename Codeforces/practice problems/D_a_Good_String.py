...
def f(s, c):
    ...
    # option 1 : left ko (c + 1) good karo and right ko c-good
    cnt_l = f(left, chr(ord(c) + 1))
    cnt_l += mid - right.count(c)

    # option 2 : right ko (c + 1) good karro and left ko c-good
    cnt_r = f(right, chr(ord(c) + 1))
    cnt_r += mid - left.count(c)
